      ...
      <div className="relative">
        ...
        <div className="absolute top-6 right-6 z-[1000] flex flex-col gap-2">
           <button 
             ...
             className={`px-4 py-2 rounded-xl backdrop-blur-xl border transition-all flex items-center gap-3 ${
               showZones ? 'bg-[#00FF9C]/10 border-[#00FF9C]/30 text-[#00FF9C]' : 'bg-white/5 border-white/10 text-white/30'
             }`}
           >
              <div className={`w-1.5 h-1.5 rounded-full ${showZones ? 'bg-[#00FF9C] shadow-[0_0_8px_#00FF9C]' : 'bg-white/20'}`} />
              <span className="text-[9px] font-black uppercase tracking-widest">Zone Overlay</span>
           </button>
           <button 
           ...
           >
              <div className={`w-1.5 h-1.5 rounded-full ${showBins ? 'bg-blue-500 shadow-[0_0_8px_#3b82f6]' : 'bg-white/20'}`} />
              <span className="text-[9px] font-black uppercase tracking-widest">Asset Layer</span>
           </button>
        </div>

        {/* Tactical Worker Overlay */}
        <div className="absolute bottom-6 right-6 z-[1000] w-80 max-h-[300px] bg-[#0D1117]/80 backdrop-blur-2xl border border-white/10 rounded-[2rem] shadow-2xl overflow-hidden flex flex-col">
           <div className="p-6 border-b border-white/5 flex items-center justify-between">
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40">Field Units</span>
              <div className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center">
                 <Radio className="w-4 h-4 text-[#00FF9C]" />
              </div>
           </div>
